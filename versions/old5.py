"""
This is version 1.4.8 Dated: 20/09/2020
There is proper summarisation of the drinks, with little bugs.
There isn't yet any mention of address or name.

The size of the window has been regulated but there are some refresh errors.
"""
...
class PickOrDelivery(wx.Panel):  # todo move this down till choices()
    def __init__(self, parent):
        super().__init__(parent)
        self.parent = parent
...
class Summary(wx.Panel):
    def __init__(self, parent):
        ...
        # print(self.GetSize())
        intro_txt.Wrap(170)
        self.summary_sizer.Add(intro_txt, 0, wx.ALIGN_LEFT, 5)
    ...
    def add(self, drink_dict=None):
        if len(drink_dict) != 0:
            ...
            serving_text = wx.StaticText(self, label=self.order)
...
class MainFrame(wx.Frame):
    def __init__(self):
        super().__init__(parent=None, title="Coffee Order", size=(672,500))
